...
export default function TeacherCourseEditor({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  const [isCreating, setIsCreating] = useState(false); // Local loading state for modal
  ...
  const handleUpdateCourse = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const res = await db.user.updateCourse(courseId, editCourseData);
      if (res.success) {
        await fetchCourse();
        setIsCourseEditOpen(false);
      } else {
        alert(res.message || "Error updating course");
      }
    } catch (err) {
      console.error(err);
  ...
  if (loading) return (
    <div className="p-20 text-center">
      ...
      <p className="text-on_surface_variant animate-pulse">Loading Atelier Workspace...</p>
    </div>
  );
    ...
    <div className="space-y-8 pb-20 max-w-5xl mx-auto">
      ...
      <header className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          <Link href="/teacher/courses" className="p-2 hover:bg-surface_container_high rounded-full transition-colors">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-surface_container_high text-on_surface rounded-full text-xs font-bold hover:bg-primary hover:text-on_primary transition-all flex items-center gap-2"
          >
            Edit Course Info
          </button>
          <button
      ...
      <div className="space-y-4">
        {modules.length === 0 ? (
          <KnowledgeCard className="p-16 text-center border-dashed">
            ...
            <p className="text-sm text-outline">Add your first module to start curating this curriculum.</p>
          </KnowledgeCard>
        ) : (
        ...
        >
          ...
          <span className="text-sm font-bold text-primary">
            {isCreating ? "Provisioning..." : "Create New Module Section"}
          </span>
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-surface_container_lowest w-full max-w-lg rounded-3xl shadow-ambient border border-outline_variant/20 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-8 border-b border-outline_variant/10">
              <h2 className="text-2xl font-bold text-on_surface font-manrope">Update Course Information</h2>
              <button onClick={() => setIsCourseEditOpen(false)} className="text-on_surface_variant hover:text-on_surface p-2 rounded-full hover:bg-surface_container_high transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleUpdateCourse} className="p-8 space-y-6">
              <div>
                ...
                <input
                  required type="text" value={editCourseData.title}
                  onChange={e => setEditCourseData({ ...editCourseData, title: e.target.value })}
                  className="w-full bg-surface_container_highest border border-outline_variant/30 rounded-xl px-4 py-4 text-sm focus:outline-none focus:border-primary/50 text-on_surface"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-surface_container_highest border border-outline_variant/30 rounded-xl px-4 py-4 text-sm focus:outline-none focus:border-primary/50 text-on_surface resize-none"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text" value={editCourseData.duration}
                ...
                <div>
                  ...
                  <input
                    type="number" min="0" value={editCourseData.price}
                    onChange={e => setEditCourseData({ ...editCourseData, price: Number(e.target.value), isPaid: Number(e.target.value) > 0 })}
                    className="w-full bg-surface_container_highest border border-outline_variant/30 rounded-xl px-4 py-4 text-sm focus:outline-none focus:border-primary/50 text-on_surface"
                  />
                </div>
              </div>
              <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setIsCourseEditOpen(false)} className="flex-1 py-4 rounded-2xl font-bold text-sm text-on_surface_variant hover:bg-surface_container_high transition-colors">
                  Cancel
                </button>
                <button type="submit" disabled={saving} className="flex-1 bg-primary text-on_primary py-4 rounded-2xl font-bold text-sm shadow-ambient hover:scale-[1.02] transition-all disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-surface_container_lowest w-full max-w-md rounded-2xl shadow-ambient border border-outline_variant/20 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-outline_variant/10">
              <h2 className="text-xl font-bold text-on_surface font-manrope">Provision Module</h2>
              <button 
