...
exports.getStudentDashboard = async (req, res) => {
  try {
    ...
    .sort({ date: 1 })
    .limit(5)
    .select("_id topic date duration batchId reminderSent status teacherId recordingExpiryDate")
  ...
  try {
    ...
    const [user, batches, todayClasses, upcomingClasses, teacherClasses] = await Promise.all([
      ...
      LiveClass.find({
        ...
        .sort({ date: 1 })
        .select("topic date duration batchId reminderSent status teacherId")
        .populate("batchId", "name")
        .lean(),
