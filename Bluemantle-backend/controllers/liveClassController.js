...
const parseScheduleDate = (value) => {
    ...
    const match = rawValue.match(DATETIME_LOCAL_PATTERN);
...
exports.scheduleClass = async (req, res) => {
    try {
        ...
        const proposedStart = parseScheduleDate(date);
...
exports.getMyClasses = async (req, res) => {
    try {
        ...
        const accessibleBatches = batches;
        ...
        const visibleClasses = classes
            ...
            .map((liveClass) => ({
                ...
                recordingUrl: liveClass.recordingUrl || (liveClass.zoomCloudRecordingUrl ? `/classes/${liveClass._id}/recording-stream` : null),
...
exports.watchRecording = async (req, res) => {
    try {
        ...
        if (liveClass.status !== "recorded" || (!liveClass.recordingUrl && !liveClass.zoomCloudRecordingUrl)) {
            await attachCloudRecording(liveClass);
        }
...
exports.streamCloudRecording = async (req, res) => {
    try {
        const { id } = req.params;
        const liveClass = await LiveClass.findById(id);
        if (!liveClass) return res.status(404).json({ success: false, message: "Class not found" });
        if (!liveClass.zoomMeetingId) {
            return res.status(404).json({ success: false, message: "Zoom cloud recording is not attached to this class." });
        }

        if (req.user.role === "student") {
            const access = await validateStudentClassAccess(liveClass, req.user.id, req.userDb);
            if (!access.ok) {
                return res.status(access.status).json({ success: false, message: access.message });
            }
        } else if (!isPrivilegedRole(req.user.role) && !(await classBelongsToTeacher(liveClass, req.user.id))) {
            return res.status(403).json({ success: false, message: "You can only watch recordings assigned to you." });
        }
        ...
        if (expiryDate < new Date()) {
            ...
            return res.status(403).json({ success: false, message: "Recording access expired." });
        }

        const recordings = await zoomHelper.getMeetingRecordings(liveClass.zoomMeetingId);
        const playableRecording = selectPlayableRecording(recordings);
        const downloadUrl = playableRecording?.download_url;

        if (!downloadUrl) {
            return res.status(404).json({ success: false, message: "Zoom cloud recording is not ready yet." });
        }

        if (req.user.role === "student") {
            const existingAttendance = await Attendance.findOne({ classId: liveClass._id, studentId: req.user.id });
            if (!existingAttendance || existingAttendance.status !== "present") {
                await Attendance.findOneAndUpdate(
                    { classId: liveClass._id, studentId: req.user.id },
                    { status: "late" },
        ...
        const streamResponse = await zoomHelper.downloadRecordingStream(downloadUrl, req.headers.range);
        ["content-type", "content-length", "content-range", "accept-ranges"].forEach((header) => {
            const value = streamResponse.headers[header];
...
exports.updateClass = async (req, res) => {
    try {
        ...
        const nextScheduledDate = date ? parseScheduleDate(date) : liveClass.date;
        ...
        if (date || duration) {
            const proposedStart = nextScheduledDate;
        ...
        if (topic || date || duration) {
            try {
                await zoomHelper.updateZoomMeeting(liveClass.zoomMeetingId, {
                    ...
                    startTime: nextScheduledDate.toISOString(),
        ...
        if (date) liveClass.date = nextScheduledDate;
