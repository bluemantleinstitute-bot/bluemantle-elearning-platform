...
const zoomStreamHeaders = (token, range) => ({
    'Authorization': `Bearer ${token}`,
    ...(range ? { Range: range } : {})
});
...
const getZoomAccessToken = async () => {
    try {
        ...
        const response = await zoomAxios.post(tokenUrl, {}, {
...
exports.createZoomMeeting = async ({ topic, startTime, duration = 60, teacherEmail }) => {
    try {
        ...
        let response;
        try {
            response = await zoomAxios.post(hostEndpoint, payload, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
            ...
            if (teacherEmail && initialErr.response) {
                ...
                response = await zoomAxios.post(`https://api.zoom.us/v2/users/me/meetings`, payload, {
...
exports.getHostZak = async (hostEmailOrId) => {
    try {
        ...
        const response = await zoomAxios.get(url, {
...
exports.downloadRecordingStream = async (downloadUrl, range) => {
    try {
        const token = await getZoomAccessToken();
        return zoomAxios.get(downloadUrl, {
            headers: zoomStreamHeaders(token, range),
            responseType: "stream",
            validateStatus: (status) => status >= 200 && status < 300
        });
    } catch (error) {
        console.error("Error streaming Zoom recording:", error.response?.data || error.message);
        throw new Error("Failed to stream Zoom cloud recording");
    }
};
/**
...
exports.updateZoomMeeting = async (meetingId, { topic, startTime, duration }) => {
    try {
        ...
        await zoomAxios.patch(url, payload, {
